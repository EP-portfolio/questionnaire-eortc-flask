"""
Gestionnaire audio optimisé pour le web (sans pygame)
Version adaptée pour le déploiement sur Render
"""
...
class AudioHandlerWeb:
    ...
    def __init__(
        ...
        # Dossier de cache permanent (à côté de audio_handler.py)
    ...
    def _get_cache_path(self, text: str, use_style_prompt: bool = True) -> Path:
        ...
        # IMPORTANT : Ne PAS inclure le préfixe de style dans le hash
        # pour correspondre au cache déjà généré
    ...
    def _synthesize_speech_gemini(
        ...
        Args:
            ...
            add_style_prompt: Ajouter des instructions pour stabiliser la voix
        """
        headers = {"Content-Type": "application/json", "x-goog-api-key": self.api_key}

        # ✅ AJOUT D'INSTRUCTIONS DE STYLE pour accent français standard
        if add_style_prompt:
            # Préfixe pour guider la voix
    ...
    def pregenerate_audio(self, texts: List[str], progress_callback=None):
        ...
        Args:
            texts: Liste des textes à pré-générer
            progress_callback: Fonction appelée avec (current, total) pour afficher la progression
        """
        if not texts:
            return
...
class VoiceRecognitionHandler:
    """Gestionnaire de reconnaissance vocale - Version web"""

    def __init__(self):
        self.recognizer = sr.Recognizer()
        self.recognition_enabled = False

        print("INFO Reconnaissance vocale configurée pour le web")
        print("INFO Utilisation de Web Speech API (JavaScript)")
    ...
    def interpret_response(self, text: str, scale: str) -> Optional[int]:
        ...
        if scale == "1-4":
            ...
            if any(word in text for word in ["très mauvais", "horrible", "terrible"]):
                ...
                word in text for word in ["plutôt bon", "plutôt bien", "assez bien"]
            ):
                ...
                word in text for word in ["très bon", "excellent", "parfait", "super"]
            ):
                print("✅ Reconnu comme: 7 (excellent)")
                return 7

        # Dictionnaire étendu avec variantes phonétiques pour le chiffre "1"
        numbers = {
            ...
            "6": 6,
            "7": 7,
            # Variantes phonétiques pour "1" (problème principal)
            "ain": 1,  # "un" mal reconnu
            "eun": 1,  # "un" mal reconnu
            "hun": 1,  # "un" mal reconnu
            "in": 1,  # "un" mal reconnu
            "eune": 1,  # "une" mal reconnu
            "eunne": 1,  # "une" mal reconnu
            # Variantes pour "2"
            "deu": 2,  # "deux" mal reconnu
            "de": 2,  # "deux" mal reconnu
            # Variantes pour "3"
            "troi": 3,  # "trois" mal reconnu
            "troy": 3,  # "trois" mal reconnu
            # Variantes pour "4"
            "quatr": 4,  # "quatre" mal reconnu
            "quat": 4,  # "quatre" mal reconnu
            # Variantes pour "5"
            "saink": 5,  # "cinq" mal reconnu
            "sink": 5,  # "cinq" mal reconnu
            # Variantes pour "6"
            "si": 6,  # "six" mal reconnu
            "sis": 6,  # "six" mal reconnu
            # Variantes pour "7"
            "set": 7,  # "sept" mal reconnu
            "cet": 7,  # "sept" mal reconnu
        }
